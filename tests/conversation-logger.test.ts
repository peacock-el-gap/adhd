...
describe("conversation-logger", () => {
  it("creates a logger with expected methods", () => {
    const logger = createConversationLog("/tmp/test", makeIdentity({ role: "generator" }));
  ...
  it("writes a timestamped log file on finalize", async () => {
    ...
    const logger = createConversationLog(dir, makeIdentity({ role: "generator", sprint: 1, attempt: 0 }));
  ...
  it("logs tool use with Bash formatting", async () => {
    ...
    const logger = createConversationLog(dir, makeIdentity({ role: "generator", sprint: 2, attempt: 1 }));
  ...
  it("generates timestamped filename without sprint/attempt", async () => {
    ...
    const logger = createConversationLog(dir, makeIdentity({ role: "planner" }));
  ...
  it("resume does not overwrite prior logs (different timestamps)", async () => {
    ...
    // First run
    const logger1 = createConversationLog(dir, makeIdentity({ role: "generator", sprint: 2, attempt: 0 }));
    ...
    // Second run (resume)
    const logger2 = createConversationLog(dir, makeIdentity({ role: "generator", sprint: 2, attempt: 0 }));
  ...
  it("exposes timestampedName for span alignment", () => {
    const logger = createConversationLog("/tmp/test", makeIdentity({ role: "generator", sprint: 1, attempt: 0 }));
