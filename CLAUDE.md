...
- **Keep `.adhd/` out of `main`, except two durable assets:** when squash-merging a harness-developed branch, drop its `.adhd/` self-development changes before committing — but **keep `.adhd/usage.json` (cost ledger) and `.adhd/regression.json` (behavioural suite)**, which are tracked on `main`. Strip the rest with `git restore --staged --worktree .adhd ':(exclude).adhd/usage.json' ':(exclude).adhd/regression.json' && git clean -fdq .adhd`. The branch's other `.adhd/` trail is preserved in its `archive/*` tag, never on `main`.
...
- **After merging, always archive the branch's development commits with a local-only tag `archive/<branch>`, then delete the topic branch (local + origin).** The squash collapses the branch's commits away from `main`; the archive tag is the only thing that keeps them reachable — whether they are the harness's `[auto-commit]`/`[adhd]` sprint trail or commits made directly on the branch. **Never push a topic branch or an `archive/*` tag to `origin`.**
